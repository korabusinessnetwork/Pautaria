...
## A11, Configuração de segurança que o CLI ignora em silêncio
*2026-08-26 · infra · **a política existia só no arquivo***

**O que aconteceu.** Ao aplicar o `supabase/config.toml` no projeto hospedado, o
`config push` reportou `auth: updated` e seguiu adiante. Fui conferir o estado
real pela Management API e a senha mínima continuava **6**, com
`password_required_characters` nulo — enquanto o arquivo dizia, em voz alta e com
comentário justificando, que o mínimo era 10 com exigência de variedade.

**A causa.** As chaves estavam na seção errada. O arquivo trazia:
...
mas o CLI lê `minimum_password_length` e `password_requirements` no nível
`[auth]`. Chave desconhecida em TOML não é erro: o parser aceita, o CLI ignora, o
push devolve sucesso. Nada em lugar nenhum reclama.

**Por que é pior que uma política ausente.** Uma política ausente é uma lacuna
conhecida, e alguém eventualmente a fecha. Esta era uma lacuna *documentada como
fechada*: o comentário no arquivo explicava o raciocínio de segurança em três
linhas convincentes, o que garante que ninguém vai conferir de novo. O projeto
acreditava exigir 10 caracteres e aceitava `123456`.

**O mesmo padrão apareceu duas vezes na mesma sessão.** O diff do push também
tentava baixar `max_frequency` de 1 min para 1 s e `otp_length` de 8 para 6 —
não por decisão de ninguém, mas porque *omitir* uma chave faz o CLI aplicar o
default dele por cima de um valor remoto mais forte. No `config push`, silêncio
não é "mantenha como está", é "use o meu padrão".

**Lição.** Não confie no sucesso de um `apply` para concluir que a configuração
foi aplicada: leia de volta o estado do servidor e compare campo a campo. E, em
arquivo de configuração declarativa, **ausência de chave é uma decisão** — toda
configuração de segurança que importa precisa estar escrita explicitamente,
mesmo quando o valor desejado parece ser o padrão.
